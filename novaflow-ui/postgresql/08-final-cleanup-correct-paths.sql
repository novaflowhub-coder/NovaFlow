-- Complete cleanup and admin setup with correct UI and backend paths
...
-- Insert pages - UI path for display, backend checks /user-management
INSERT INTO user_management.pages (id, name, path, description, created_by) VALUES
('page-users', 'Users', '/user-management', 'User management (backend checks this path)', 'system');
...
SELECT 'Page Setup' as check_type, name, path FROM pages WHERE id = 'page-users';
...
-- Test backend authorization (what UserController checks)
SELECT 'Backend Auth Check' as check_type,
       EXISTS (
           SELECT 1
